...
func main() {
	usage := `欢迎使用时源科技档案袋归位工具！

    本程序可将输入的装有 身份证号.jpg的档案袋文件 的目录下全部 身份证号.jpg 文件移动到输入的项目目录中对应的身份证号文件夹的“目录”内。
    ...
    *注意！
    ...
    Usage:  根据提示输入两个文件夹的路径即可开始。
		...
		如：
		    输入包含所有身份证号.jpg档案袋文件的文件夹的绝对路径或其与程序所在文件夹的相对路径并按回车：D:\project\封皮
	...
	// 正文开始
	bindingsDir := ""
	fmt.Print("输入包含所有身份证号.jpg档案袋文件的文件夹的绝对路径或其与程序所在文件夹的相对路径并按回车：")
	fmt.Scanln(&bindingsDir)
	jpgList, err := getBindings(bindingsDir) // 读取jpg列表
	if err != nil {
		log.Println("读取档案袋文件夹失败：", err)
	}
	projectDir := ""
	...
	for _, jpg := range jpgList {
		...
		newPath := path.Join(aimDir, jpgName+"-0(1"+ext)
